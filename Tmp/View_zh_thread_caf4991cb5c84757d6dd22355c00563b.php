 ...
 <div class="slimScrollRail" style="width: 0px; height: 100%; position: absolute; top: 0px; display: none; border-radius: 7px; opacity: 0.2; z-index: 90; right: 1px; background: rgb(51, 51, 51);"></div></div></aside>
  ...
  <div class="main-container">
    <div class="padding-md">
      <form method="get">
      <input type="hidden" name="s" value="admin/thread">
       ...
       <div class="form-group">
         ...
         <button type="submit" class="btn btn-success btn-sm">筛选</button> <a href="?top=6" class="btn btn-success btn-sm">审核</a><a href="?top=7" class="btn btn-success btn-sm">精华</a><a href="?top=8" class="btn btn-success btn-sm">栏目置顶</a><a href="?top=9" class="btn btn-success btn-sm">全站置顶</a>
      ...
      <table class="table table-striped table-bordered m-top-md" id="dataTable">
       ...
       <tr>
         ...
         <td><?php echo $v['id'];?></td>
         <td><small class="badge badge-success badge-square bounceIn animation-delay2 m-left-xs pull-right"><?php echo $v['fid'];?></small></td>
         ...
         <td>
          <a href="/thread/<?php echo $v['id'];?>.html?admin" target="_blank"><?php echo $v['title'];?></a>
         </td>
         <td><?php echo date("Y-m-d H:i:s",$v['atime']); ?></td>
         
         <td>
          <p>评论：<?php echo $v['posts'];?></p>
          <p>浏览：<?php echo $v['views'];?></p>
          <p>附件：<?php echo $v['files'];?></p>
         
         </td>
         <td>
          <p>赞：<?php echo $v['goods'];?></p>
          <p>踩：<?php echo $v['nos'];?></p>
         </td>
       </tr>
       <?php endforeach ?>
  				</tbody>
    ...
    <div class="smart-widget-body">
<?php if ($i>9): ?><a href="/admin/thread/<?php echo $pagetime;?>.html" class="btn btn-primary btn-lg btn-block">下一页<i class="fa fa-angle-double-right m-left-xs"></i></a><?php else: ?><button class="btn btn-lg btn-block">完毕</button><?php endif ?>
    </div>
