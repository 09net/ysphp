...
<div class="wrap-box">
{if isset($fdata)}
<ol><li><a href="/">{lag 首页}</a></li><li class="fa fa-chevron-right"><a href="/f/{$fdata['id']}.html">{$fdata['name']}</a></li><li class="fa fa-chevron-right">{lag 发布主题}</li></ol>
{else}
<ol><li><a href="/">{lag 首页}</a></li><li class="fa fa-chevron-right">{lag 发布主题}</li></ol>
{/if}
</div>
<div class="wrap-box">
<div class="row">
<div class="col-xs-3">
{if isset($fdata)}<input id="fid" type="hidden" value="{$fdata['id']}" />{$fdata['name']}{else}<select id="fid" type="text" class="input" style="width:150px;margin-bottom:10px"><option value="-1">{lag 选择}</option>{if $forum}
{foreach $forum as $key => $v}<option value="{$v['id']}">{$v['name']}</option>{/foreach}{/if}</select>{/if}
</div>
<div class="col-xs-9">
<input type="text" id="title" class="input-text " placeholder="{lag 请填写标题}">
</div></div>
