{include h}<div class="container"><div id="main"><div class="wrap-box"><ol><li><a href="/">{lag 首页}</a></li></ol>
</div><div class="wrap-box t-info"><div class="head">
<h1 id="titlefy"><a href="/comment/more/{$thread_data['id']}.html">{$thread_data['content']}</a></h1>
...
<input name="pid" id="pid" type="hidden" value="{$thread_data['id']}" />
{if IS_LOGIN}
<textarea id="content" name="content" class="input-text"></textarea>
<p><button type="submit" class="btn btn-primary" id="post_post">{lag 评论}</button><span id="acom">@{$thread_data['id']}</span></p>
{else}
