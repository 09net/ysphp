...
if($fid != -1){/*栏目*/
  if($mode>0) $data['AND']=array('domain'=>$fid,'fid'=>$mode,'btime[<]'=>$pageid);else $data['AND']=array('domain'=>$fid,'btime[<]'=>$pageid);
}elseif($uid != 0){/*UID*/
$data['AND']=array('uid'=>$uid,'btime[<]'=>$pageid);
 
 }else{/*多语言*/
  if($mode>0)  $data['AND']=array('lang'=>NOW_LANG,'fid'=>$mode,'btime[<]'=>$pageid); else $data['AND']=array('lang'=>NOW_LANG,'btime[<]'=>$pageid);
 }

if($mcache!=false) $data2=$mcache->get('tl'.$uid.'_'.$mode.$fid.NOW_LANG.$pageid);
...
if($mcache!=false) $data2=$mcache->get(md5('ts'.$pageid.$key[0]));
if(empty($data2)) $data2=$this->select($this->sqlme,array('LIMIT'=>array($listnum*$pageid,$listnum),'MATCH'=>array('columns'=>array('pinyin'),'keyword'=>$key[0].' IN NATURAL LANGUAGE MODE')));
