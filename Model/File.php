...
if($mcache!=false) $data2=$mcache->get('fl'.$uid.'_'.$mode.$fid.NOW_LANG.$pageid);
if(empty($data2)) $data2=$this->select($this->sqlme,$data);
$this->format($data2,$mcache);
if($mcache!=false and $data2) $mcache->set('fl'.$uid.'_'.$mode.$fid.NOW_LANG.$pageid,$data2);
return $data2;
 }
...
if($data=$this->find($this->sqlme,array('id'=>$id)) and $mcache!=false) $mcache->set(__CLASS__.$id,$data);
return $data;
}
...
if(empty($thread_list)) return false;
 ...
 if($key) $v['filename']= str_ireplace($k1,$k2,$v['filename']);
 }
