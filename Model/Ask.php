...
if($mcache!=false) $data2=$mcache->get('al'.$uid.'_'.$mode.$fid.NOW_LANG.$pageid);
if(empty($data2)) $data2=$this->select($this->sqlme,$data);
$this->format($data2,$mcache);
if($mcache!=false and $data2) $mcache->set('al'.$uid.'_'.$mode.$fid.NOW_LANG.$pageid,$data2);
return $data2;
 }
...
if($mcache!=false) $data2=$mcache->get(md5('als'.$pageid.$key[0]));
...
if($mcache!=false and $data2) $mcache->set(md5('als'.$pageid.$key[0]),$data2);
return $data2;
}
