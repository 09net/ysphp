...
function new_chat(title, msg, w, h,uid,user,avatar,ps){ 
 ...
 var msgObj=$('<div uid="'+uid+'" avatar="'+avatar+'" user="'+user+'" class="lt-dlg-box" id="lt-box-'+uid+'" style="z-index:'+(zindex++)+';top:'+($(window).height()-h)/2+'px;left:'+($(window).width()-w)/2+'px;width:'+w+'px;height:'+h+'px;"></div>');
 $('body').append(msgObj);
 ...
 var h4 = $('<h4 style="margin-bottom: 0;"><img src="'+avatar+'"><span class="lt-name">'+user+'</span><span class="lt-ps">'+ps+'</span></h4>');
...
function get_old_chat(uid,user,avatar,bool){
 ...
 for(o in e.reverse()){
 if(e[o].uid1 == uid){
 add_lt(uid,'right',window.YS_user,e[o].content,e[o].time,window.YS_avatar);
 }
 else{
...
function send_lt(uid,obj){
 ...
 add_lt(uid,'right',window.YS_user,$("#lt-text-"+uid).val(),new Date().getHours() + ":"+ new Date().getMinutes() +":"+ new Date().getSeconds() ,window.YS_avatar);
 $("#lt-text-"+uid).val('');
 $("#lt-text-"+uid).removeAttr('disabled');
 $(obj).removeAttr('disabled');
 $("#lt-text-"+uid).focus();
...
function load_friend(){
 ...
 for(o in e){
 if(e[o].state==0){
 html0 += '<li><a onclick="new_chat(\'title\',\'ssss\',444,465,'+e[o].uid+',\''+e[o].user+'\',\''+e[o].avatar+'\',\''+((e[o].ps==null)?'':e[o].ps)+'\')" >'+
 '<img src="'+e[o].avatar+'" class="img-circle" alt="user avatar">'+
 ...
 html2 += '<li><a onclick="new_chat(\'title\',\'ssss\',444,465,'+e[o].uid+',\''+e[o].user+'\',\''+e[o].avatar+'\',\''+((e[o].ps==null)?'':e[o].ps)+'\')" >'+
 '<img src="'+e[o].avatar+'" class="img-circle" alt="user avatar">'+
 '<div class="chat-detail m-left-sm"><div class="chat-name">'+e[o].user+
 '</div><div class="chat-message">'+((e[o].ps==null)?'':e[o].ps)+'</div>'+
 '</div><div class="chat-status"><span class="friend-'+(e[o].ol?'zx':'lx')+'"></span></div>'+
 ...
 html3 += '<li><a onclick="new_chat(\'title\',\'ssss\',444,465,'+e[o].uid+',\''+e[o].user+'\',\''+e[o].avatar+'\',\''+((e[o].ps==null)?'':e[o].ps)+'\')" >'+
 '<img src="'+e[o].avatar+'" class="img-circle" alt="user avatar">'+
 '<div class="chat-detail m-left-sm"><div class="chat-name">'+e[o].user+
 '</div><div class="chat-message">'+((e[o].ps==null)?'':e[o].ps)+'</div>'+
 '</div><div class="chat-status"><span class="friend-'+(e[o].ol?'zx':'lx')+'"></span></div>'+
 ...
 window.friend_pm = 0;
setTimeout("timeout_pm()",pm_time*int_pm); 
 },
error:function(){}
 })
}
...
function add_friend_li(uid){
 ...
 success:function(e){
 if(e.code==200){
 var html = '<li><a onclick="new_chat(\'title\',\'ssss\',444,365,'+uid+',\''+e.info.user+'\',\''+e.info.avatar+'\')">'+
 '<img src="'+e.info.avatar+'" class="img-circle" alt="user avatar">'+
 '<div class="chat-detail m-left-sm"><div class="chat-name">'+e.info.user+
 '</div><div class="chat-message">'+langfun('签名')+'</div>'+
