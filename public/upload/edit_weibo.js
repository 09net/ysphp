function insertvideo(a) { 
 window.upvi=true;
$('#img').append('<video width="300" height="140" src="'+a+'" controls="controls"></video><p>&nbsp;</p><p>&nbsp;</p>')
$('#vidv').val($('#vidv').val()+','+a);
}
...
if (file) uploadFile(type,id);
}
function uploadFile(type,id) {
if(window.hy_edit_upload){swal("请等待第一个上传完成!");return false;}
window.upei=window.upei+1;
if(window.upei>5){swal("最多可以上传5个文件");return false;}
if (type=="uploadfile" && window.upvi){swal("只允许上传一个视频!");return false;}
 window.hy_edit_upload = true;
 if(jdt != null){jdt.remove();jdt =null;}
