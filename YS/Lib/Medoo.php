...
class Medoo{
 ...
 if ($this->debug_mode){echo $query;$this->debug_mode = false;return false;}
 array_push($this->logs, $query);
 $a = microtime(TRUE);
 $re = $this->pdo->query($query);
 if (C('DEBUG_PAGE')){
 \YS\YS::SQL_LOG($query . ' [耗时] ' . round(microtime(TRUE) - $a, 4) . 'ms');
}
 return $re;
}
 ...
 protected function where_clause($where)
{
 ...
 $where_clause .= ($where_clause != '' ? ' AND ' : ' WHERE ') . ' MATCH (`' . str_replace('.', '"."', implode($MATCH['columns'], '", "')) . '`) AGAINST (' . $this->quote($MATCH['keyword']) . '  IN BOOLEAN MODE)';
}
}
...
return $query ? $query->fetchAll(is_string($columns) && $columns != '*' ? PDO::FETCH_COLUMN : PDO::FETCH_ASSOC) : false;
 ...
 $lastId = array();
 if (!isset($datas[0])) $datas = array($datas);
...
if(!is_array($data)) continue;
 ...
 public function insertAll($table, $columns, $datas)
{
 if (empty($datas) || empty($columns)){
 return false;
}
 if (!isset($datas[0])){
 $datas = array($datas);
 ...
 $cc = array();
 foreach ($columns as $key => $value){
 $cc[] = $this->column_quote($value);
}
 $this->exec('INSERT INTO `' . $this->prefix . $table . '` (' . implode(', ', $cc) . ') VALUES ' . implode(', ', $val));
 return true;
}
 // 升级数据
