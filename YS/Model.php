...
class Model {
 ...
 $prefix = $sql_more_bool ? $sql_more['SQL_PREFIX'] : C("SQL_PREFIX");
 $option = $sql_more_bool ? $sql_more['SQL_OPTION'] : C("SQL_OPTION");
 $a = microtime(TRUE);
 $pdo_obj[$key] = new Lib\Medoo(array(
 // 必须配置项
 ...
 YS::SQL_LOG('连接'.$database_type.' '.$database_name.'数据库 [耗时] ' .round(microtime(TRUE) - $a, 4).'ms');
 }
