...
// console.log("event:"+e);
 ...
 if (feedback.fid.value == "0") {
 return mui.toast(l[lang]['小组为空']);
 }
 if (feedback.question.value.length < 5) {
 return mui.toast(l[lang]['内容太少'])
 }
 
 //判断网络连接
 if(plus.networkinfo.getCurrentType()==plus.networkinfo.CONNECTION_NONE){
 return mui.toast(l[lang]['网络中断']);
 }
 feedback.send(mui.extend({}, feedback.deviceInfo, {
 content: feedback.question.value,
 fid: feedback.fid.value,
 images: feedback.files,
 gold: feedback.gold.value,
...
var data = eval("("+ upload.responseText +")");
} 
mui.alert(data['info']);
if(data['error']){openurl(data['size']+'.html?id='+data['id'],data['id']);feedback.clearForm();}
}else{
mui.alert(l[lang]['失败']);
}
